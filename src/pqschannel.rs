...
impl<Stream: Read + Write> PqsChannel<Stream> {
  fn new_inner(mut stream: Stream, plain_secret: PlainText) -> std::io::Result<Self> {
    ...
    let mut read_iv = write_iv.clone();
  ...
  pub fn new(mut stream: Stream, context: &PqsContext) -> std::io::Result<Self> {
    match context {
      PqsContext::Client {
        ca_cert,
        cert: _,
        ...
        if !cert.verify(&ca_cert.pub_keys) {
          return Err(std::io::Error::new(
            ErrorKind::NotConnected,
            "Certificate validation failed for server",
          ));
        }
        let (shared_secret, plain_secret) = cert.pub_keys.encapsulate();
      ...
      PqsContext::Server {
        ...
        cert.serialize(&mut stream)?;
        let shared_secret = Encapsulated::deserialize(&mut stream)?;
        let plain_secret = secret.decapsulate(&shared_secret);
        Self::new_inner(stream, plain_secret)
      }
    }
  }
